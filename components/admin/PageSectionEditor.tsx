...
export interface PageSectionEditorProps {
  ...
  viewHref: string;
  /** Optional link to manage list content (e.g. "Manage posts" for blog). */
  manageHref?: string;
  manageLabel?: string;
  /** Optional back link (e.g. "← Pages" for About). */
  backHref?: string;
  backLabel?: string;
}
  ...
  return (
    <div className="p-8">
      {backHref && (
        <Link
          href={backHref}
          className="mb-4 inline-block text-sm text-muted hover:text-[var(--foreground)]"
        >
          {backLabel}
        </Link>
      )}
      ...
      <div className="mt-6 mx-auto max-w-5xl rounded-xl border border-border bg-[var(--background)] p-4 sm:p-6">
